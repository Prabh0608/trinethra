...
app.post("/analyze", async (req, res) => {
  ...
  const prompt = `
              ...
              - On the basis of your understanding, extracted quotes compare it with the description in ${rubricScore.rubric} and give them score. IMPORTANT: understand deeply before scoring especially between 6 and 7 score (take a look at ${rubricScore.rubric.criticalBoundary})
              ...
              - On the basis of the supervisor's description connect the fello's work with its most suitable KPI present in ${rubricScore.kpis} and use the labels for the same also justify why you choose this
              ...
              - Keep in mind that supervisors are honest but biased. like Helpfulness bias, Presence bias, Halo/horn effect , Recency bias these might change the score so pay attention on this
              - On the basis of above rules fill in the 5 valid json field. IMPORTANT NOTE: inside each of these 5 field you may add any number of subfield that will help the user easily understand the output and the data if you do so keep in mind to name all of these subfield(dont use question and answer for gaps)
              ...
              - If unsure between two scores, prefer the lower score unless strong evidence supports the higher one.
